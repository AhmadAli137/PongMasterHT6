...
esp_err_t wifi_link_connect(uint32_t timeout_ms)
{
    ...
    /* The whole reason Wi-Fi is in the BENCH firmware: PS_NONE holds the draw
     * at ~100 mA+ so the DAOKI boost stays awake through the idle gaps between
     * bench phases (otherwise it decides we're unplugged and cuts power). */
...
bool wifi_link_up(void) { return s_up; }
...
uint32_t wifi_link_reconnects(void) { return s_reconnects; }
